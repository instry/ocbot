...
const zhCNTranslations: Record<string, string> = {
  ...
  'No pairing needed for the managed browser.': '托管浏览器模式无需配对。',
  'Leave Profile on Auto-detect to use an isolated browser that Ocbot controls directly.': '将配置文件保持为“自动检测”，Ocbot 就会直接控制隔离的代理浏览器。',
  'Pairing is only required when you attach your own Chromium profile.': '只有在连接你自己的 Chromium 配置文件时才需要配对。',
  'To reuse your own logins and cookies, enable remote debugging in the selected browser and approve the first attach prompt.': '如果要复用你自己的登录态和 Cookie，请先在选中的浏览器里开启远程调试，并同意首次连接提示。',
  'Open pairing page': '打开配对页面',
  'Pair browser': '配对浏览器',
  'Pairing...': '正在配对...',
  'Browser paired. Ocbot can now control the selected profile.': '浏览器已配对，Ocbot 现在可以控制所选配置文件。',
  'Pairing request sent. If the browser shows a consent prompt, approve it to finish connecting.': '已发起配对请求。如果浏览器弹出授权提示，请点击同意以完成连接。',
  'Pairing failed. Enable remote debugging on the selected browser and approve the browser prompt, then try again.': '配对失败。请先在所选浏览器中开启远程调试，并同意浏览器提示后再重试。',
  'Remote debugging is ready. Ocbot will attach to this profile on the next browser task.': '远程调试已经可用。Ocbot 会在下一次浏览器任务时连接到这个配置文件。',
  'Remote debugging is not reachable on port 9222 yet. Launch the selected browser with remote debugging enabled, then try again.': '端口 9222 上的远程调试暂时还不可访问。请先用已开启远程调试的方式启动所选浏览器，然后再试一次。',
  'Failed to check the browser debug connection.': '检查浏览器调试连接失败。',
  'Failed to open the pairing page in the selected browser.': '无法在所选浏览器中打开配对页面。',
  'How to pair': '如何配对',
  'Choose the matching Chromium profile.': '选择对应的 Chromium Profile。',
  'Open the pairing page from here.': '先点这里的“打开配对页面”。',
  'Enable remote debugging in that browser.': '在浏览器里开启 remote debugging。',
  'Come back to Ocbot and click Pair browser.': '回到 Ocbot 后点击“配对浏览器”。',
  'Approve the first consent prompt in the browser.': '浏览器弹出首次授权时点同意。',
  Refresh: '刷新',
  Cancel: '取消',
  Save: '保存',
  'Saving...': '正在保存...',
