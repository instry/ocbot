...
const zhCNTranslations: Record<string, string> = {
  ...
  'Clear local runtime state, saved channel credentials, logs, and cached setup, then restart the app.': '清空本地运行时状态、已保存的频道凭证、日志和缓存的初始化数据，然后重启应用。',
  'Reset local data and restart Ocbot? This removes local runtime state, saved channel credentials, logs, and cached setup.': '确定要重置本地数据并重启 Ocbot 吗？这会删除本地运行时状态、已保存的频道凭证、日志和缓存的初始化数据。',
